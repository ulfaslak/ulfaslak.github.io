...
var PATH_dataset = "sendtu"
var PATH_method = "NFC"
var PATH_filename = "datasetMon"
...
var main = function() {
    // Load dataset and start visualization inside
    //
    ...
    PATH = "data_" + PATH_dataset + "_" + PATH_method + "/" + PATH_filename + ".json"
    d3.json(PATH, function(ds_loaded) {
        spinnerw1.stop();
        spinnerw2.stop();
...
var weekdayButtons = function() {
    // Radio buttons to change weekday dataset
    //
    $('#weekdayButtonGroup .btn').on('click', function(e) {
        $('#weekdayButtonGroup .active').removeClass('active');
        $(this).addClass('active');

        // Update data source
        PATH_filename = e.toElement.id
...
var methodButtons = function() {
    // Radio buttons to change clustering method
    //
    $('#methodButtonGroup .btn').on('click', function(e) {
        $('#methodButtonGroup .active').removeClass('active');
        $(this).addClass('active');

        // Update data source
        PATH_method = e.toElement.id
...
var datasetButtons = function() {
    // Radio buttons to change clustering method
    //
    $('#datasetButtonGroup .btn').on('click', function(e) {
        $('#datasetButtonGroup .active').removeClass('active');
        $(this).addClass('active');

        // Update data source
        PATH_dataset = e.toElement.id
...
var polygons = function() {
    ...
    // Hovering and clicking behavior //
    // ------------------------------ //
    ...
    // Draw on canvas // (Important stuff happens here)
    // -------------- //
    ...
    for (var i=0; i<grid_ticks_keys.length; i++) {
        key = grid_ticks_keys[i] - 1  // -1 because it looks better.
        svg_w1.append("line")
    ...
    svg_w1.selectAll("text")
        ...
        .text(function(d) { 
            ...
            else {hour -= 12; period = " PM"}
            if (hour + period == "9 AM") {
                if (time_step == null) {
                    time_step = parseInt(d)
                }
            }
            return hour + period
        })
        ...
        if (grp_size >= size_thresh) {
            ...
            if (keep_nodes.length == 0) {
                ...
                linked_nodes = dataset['layer_networks']['data'][time_step]['links_dict'][id]
                    .filter(function(n) {
                        return keep_nodes.indexOf(String(n)) != -1;
                    })

                if (linked_nodes.length > 0) {
                    linked_nodes_objects = nodes.filter(function(i) {
                        return linked_nodes.indexOf(parseInt(i.id)) != -1;
                    });
                    x_vals = linked_nodes_objects.map(function(n) {
                        return n.x;
                    });
                    y_vals = linked_nodes_objects.map(function(n) {
                        return n.y;
                    });
...
function updateGraph(dat) {
    ...
    for (var i in remove_links) {
        if (i == "diff") continue;
        graph.removeLink(remove_links[i]['source'], remove_links[i]['target'])
    }
